...
async def handle(request):
    ...
    # print(request.headers.get('X-FORWARDED-FOR', None))
    print(get_id)
...
async def get_stats(request):
    ...
    html_data = f'<html></head><body style="background-color: #161616;color: #DCDCDC;"><pre style="word-wrap: break-word; white-space: pre-wrap;">{text_data}</pre><a style = "color: #DCDCDC;" href="{link}">{link_text}</a></body></html>'
...
async def get_unique_stats(request):
    sql_queries = {
        ...
        'unique_month': "SELECT COUNT(DISTINCT user_id) FROM visits WHERE strftime('%Y-%m', timestamp) = strftime('%Y-%m', 'now')",
        'unique_year': "SELECT COUNT(DISTINCT user_id) FROM visits WHERE strftime('%Y', timestamp) = strftime('%Y', 'now')",
...
def get_html_data_handle():
    link_stats = 'http://104.248.81.33:8080/stats'
    link_text_stats = 'Get stats\n'
    link_unique_stats = 'http://104.248.81.33:8080/unistats'
